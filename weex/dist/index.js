...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('text', {
    staticClass: ["message"]
  }, [_vm._v("Now, let's goooooooooooooooooooooooo.")])
},staticRenderFns: []}
...
__vue_options__.__file = "D:\\Weex\\fproject\\src\\components\\game.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-30437ce2"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    ...
    methods: {
        ...
        aotoPlay: function aotoPlay() {
            ...
            if (this.autoPuzzles(Setting).canReach(Setting.originalNode, Setting.resultNode)) {
                console.log('走你');
        ...
        // 自己玩执行
        autoPuzzles: function autoPuzzles(Setting) {
            var that = this;
            var queueArr = []; //队列
            var hashObj = {}; //hash
            var NodeObj = {}; //记录节点
            var originalNode = Setting.originalNode; //原始节点
            var originalNodeStr = originalNode.toString().split(",").join("");
            var resultNode = Setting.resultNode; //结果节点
            var resultNodeStr = resultNode.toString().split(",").join("");
            var isFind = false;
            var delay = Setting.delayTime || 1000; //动画延迟
            return {
                // 是否可达
                canReach: function canReach(originalNode, resultNode) {
                ...
                searchPath: function searchPath() {
                    ...
                    var _loop = function _loop() {
                        ...
                        var currentNode = queueArr.shift(),
                            currentNodeStr = currentNode.toString().split(",").join("");
                        if (resultNodeStr === currentNodeStr) {
                            var path = [];
                            var pathLength = 0;
                            var resultPath = [];
                            for (var v = resultNodeStr; v != originalNodeStr; v = NodeObj[v]) {
                                path.push(hashObj[v]);
                            }
                        ...
                        // return;
                        var result = _this3.getChildNodes(currentNode);
                ...
                // 返回步骤信息
                returnStep: function returnStep(path) {
                    var _this = this;
                    path.forEach(function (item, index) {
                        setTimeout(function (node) {
                            _this.readerDom(node);
                        }.bind(_this, item), index * delay);
                    });
                },

                // 渲染dom
                readerDom: function readerDom(node) {
                    var nodeArr = node.toString().split(",");
                    that.puzzles = nodeArr;
                    that.pass();
                },

                // 获取子节点
                getChildNodes: function getChildNodes(currentNode) {
                ...
                // 获取方向
                getDirection: function getDirection(target, rowNum, colNum) {
                    var direction = [];
                    if (!target.x) {
                ...
                // 改变位置
                changePos: function changePos(node, target, direction) {
                    if (direction.length) {
                        var childNodesArr = [];
        ...
        // 点击方块
        clickBlock: function clickBlock(index) {
        ...
        pass: function pass() {
            if (this.puzzles[8] === "0" || this.puzzles[8] === 0) {
                var newPuzzles = this.puzzles.slice(0, 8);
...
module.exports = {
  ...
  "cell": {
    ...
    "marginBottom": "1",
    "marginLeft": "1",
    "MozBoxShadow": "0px 0px 3px #333333",
    "WebkitBoxShadow": "0px 0px 3px #333333",
    "boxShadow": "0px 0px 3px #333333",
    "backgroundColor": "#996666"
  },
  "cells": {
    "backgroundColor": "#ffffff",
    "MozBoxShadow": "0px 0px 0px #333333",
    "WebkitBoxShadow": "0px 0px 0px #333333",
